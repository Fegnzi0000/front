...
独立的管理员 Web SPA，用于运营概况、普通用户账号状态管理和操作审计。该项目不属于微信小程序，不提供普通用户业务详情或管理员角色管理。普通用户临时密码操作已移除，微信用户邮箱可为空。改造与配套后端版本见 [团队交接](../docs/团队同步与改造交接说明.md)。
...
- 普通用户仅微信登录，不再提供临时密码操作；管理员保留独立账号密码及本人改密功能。
