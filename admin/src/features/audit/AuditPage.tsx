...
type FilterForm = { adminAccount?: string; targetUserNickname?: string; action?: AuditAction; result?: AuditResult; dateRange?: [Dayjs, Dayjs] }
...
export default function AuditPage() {
  ...
  const columns: TableColumnsType<AuditLog> = [
    ...
    { title: '目标用户', key: 'target', width: 260, render: (_, item) => item.targetUser?.nickname ?? '—' },
  ...
  return (
    <Space orientation="vertical" size={16} className="page-stack">
      <Card className="filter-card">
        <Form<FilterForm> form={form} layout="inline" onFinish={submit} initialValues={{ adminAccount: query.adminAccount, targetUserNickname: query.targetUserNickname, action: query.action, result: query.result, dateRange: query.startDate && query.endDate ? [dayjs(query.startDate), dayjs(query.endDate)] : undefined }}>
      ...
      <Drawer title="审计详情" size={480} open={Boolean(selected)} onClose={() => setSelected(null)}>
        {selected && <Descriptions column={1} bordered size="small">
          ...
          <Descriptions.Item label="目标用户">{selected.targetUser?.nickname ?? '—'}</Descriptions.Item>
