...
type FilterForm = { adminUserId?: string; targetUserId?: string; action?: AuditAction; result?: AuditResult; dateRange?: [Dayjs, Dayjs] }
...
function queryFromParams(params: URLSearchParams): AuditQuery {
  return {
    adminUserId: params.get('adminUserId') || undefined,
...
export default function AuditPage() {
  ...
  const submit = (values: FilterForm) => {
    ...
    if (values.adminUserId?.trim()) next.adminUserId = values.adminUserId.trim()
    if (values.targetUserId?.trim()) next.targetUserId = values.targetUserId.trim()
  ...
  const columns: TableColumnsType<AuditLog> = [
    ...
    { title: '管理员', key: 'admin', width: 240, render: (_, item) => <Space orientation="vertical" size={0}><Typography.Text>{item.admin.email ?? '历史账号'}</Typography.Text><Typography.Text type="secondary" ellipsis>{item.admin.id}</Typography.Text></Space> },
    { title: '目标用户', key: 'target', width: 240, render: (_, item) => item.targetUser ? <Space orientation="vertical" size={0}><Typography.Text>{item.targetUser.email ?? '历史账号'}</Typography.Text><Typography.Text type="secondary" ellipsis>{item.targetUser.id}</Typography.Text></Space> : '—' },
  ...
  return (
    <Space orientation="vertical" size={16} className="page-stack">
      <Card className="filter-card">
        <Form<FilterForm> form={form} layout="inline" onFinish={submit} initialValues={{ adminUserId: query.adminUserId, targetUserId: query.targetUserId, action: query.action, result: query.result, dateRange: query.startDate && query.endDate ? [dayjs(query.startDate), dayjs(query.endDate)] : undefined }}>
          <Form.Item label="管理员 ID" name="adminUserId"><Input allowClear placeholder="UUID" /></Form.Item>
          <Form.Item label="目标用户 ID" name="targetUserId"><Input allowClear placeholder="UUID" /></Form.Item>
      ...
      <Drawer title="审计详情" size={480} open={Boolean(selected)} onClose={() => setSelected(null)}>
        {selected && <Descriptions column={1} bordered size="small">
          ...
          <Descriptions.Item label="管理员">{selected.admin.email ?? '历史账号'}</Descriptions.Item>
          <Descriptions.Item label="目标用户">{selected.targetUser?.email ?? '—'}</Descriptions.Item>
