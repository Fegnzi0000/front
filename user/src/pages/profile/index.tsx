...
import { getProfileFoodActions, getWechatProfilePresentation } from '../../domain/core'
...
export default function ProfilePage() {
  ...
  const preferenceCount = (preferences?.tastePreferences.length??0)+(preferences?.dietaryRestrictions.length??0)+(preferences?.dislikes.length??0)
  const tags = [...new Set(foods.flatMap((food) => food.tags))]
  const identity = getWechatProfilePresentation(user ?? {})
  const logout = async () => {
  ...
  return <View className='page profile-page'>
    ...
    <View className='profile-hero' onClick={() => Taro.navigateTo({ url: '/pages/profile/edit/index' })}><View className='profile-avatar'>饭</View><View className='profile-copy'><Text className='profile-name'>{identity.nickname}</Text><Text className='profile-account'>{identity.accountLabel}</Text><Text className='profile-edit'>编辑昵称 ›</Text></View></View>
    <Text className='group-title'>饮食设置</Text><View className='menu-stack'><ActionTile icon='¥' title='每日预算' note={preferences?.budgetEnabled ? `已设置 ¥${preferences.dailyBudget}/天` : '暂未启用预算'} onClick={() => Taro.navigateTo({ url: '/pages/budget/index' })} /><ActionTile icon='味' title='口味偏好与忌口' note={preferenceCount ? `已设置 ${preferenceCount} 项` : '忌口、不喜欢和口味偏好'} onClick={() => Taro.navigateTo({ url: '/pages/preferences/index' })} /></View>
    ...
    <Text className='group-title'>账号与系统</Text><View className='menu-stack'><ActionTile icon='锁' title='账号与安全' note='微信身份、数据权利与注销账号' onClick={() => Taro.navigateTo({ url: '/pages/account/security/index' })} /><ActionTile icon='设' title='应用设置' note='老虎机音效、隐私与本地数据' onClick={() => Taro.navigateTo({ url: '/pages/settings/index' })} /><ActionTile icon='AI' title='AI 功能预览' note='第二阶段组件展示，不调用模型' onClick={() => Taro.navigateTo({ url: '/pages/ai/preview/index' })} /></View>
